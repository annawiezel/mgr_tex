...
\begin{document}
	...
	\vspace{0.35cm}\\W pracy wykorzystano dane o polu magnetycznym Ziemi publikowane na stronie programu INTERMAGNET. Za³¹czam zatem specjalne podziêkowania:
	\vspace{0.35cm}\\\textit{\textcolor{red}{ACKNOWLEDGEMENTS}
	...
	\begin{uw}\label{HSsp} \cite{HK}
		...
		\end{equation}
		gdzie ortonormalne $\upsilon_j$ s¹ \textbf{funkcjami w³asnymi} $\mathit{\Psi}$, tj. $\mathit{\Psi}(\upsilon_j)=\lambda_j \upsilon_j$ \textcolor{red}{[a wartoœci w³asne $\lambda$?]}. Funkcje $\upsilon_j$ mog¹ byæ rozszerzone do bazy, przez dope³nienie ortogonalne podprzestrzeni rozpiêtej przez oryginalne $\upsilon_j$. Mo¿emy zatem za³o¿yæ, ¿e funkcje $\upsilon_j$ w \eqref{eq:fw} tworz¹ bazê, a pewne wartoœci $\lambda_j$ mog¹ byæ równe zero.
	\end{uw} 
	...
	%
	Przestrzeñ $L^2=L^2(T)=L^2(T,\mathcal{B}, \lambda)$ nad pewnym przedzia³em $T \subset \mathbb{R}$ jest zbiorem klas \textcolor{red}{abstrakcji?} mierzalnych funkcji rzeczywistych ca³kowalnych z kwadratem okreœlonych na $T$, tj.
	...
	\textcolor{red}{[opisaæ $\mathcal{B},\lambda$?]}\\
	...
	%
	\begin{tw}\cite{W}
		\\Niech $(T, \mu)$ bêdzie przestrzeni¹ z miar¹ \textcolor{red}{[zamieniæ $T$?]} i niech $\psi(t,s)$ bêdzie mierzaln¹ funkcj¹ na $T \times T$. Zdefiniujmy
		\[ \mathit{\Psi} x (t) = \int_T \psi (t,s) x(s) d\mu(s), \quad \textcolor{red}{x \in L^p}. \]
		Jeœli $1 < p < \infty$ oraz istnieje mierzalna dodatnia funkcja $y$ na $T$ oraz sta³e $a$, $b$ takie ¿e dla $\frac{1}{p}+\frac{1}{q}=1$ mamy
		\\\textcolor{red}{[coœ wiêcej o mierze $\mu$? $a,b>0$?]}
		\begin{equation}\label{eq:zal1}
		\int_T  | \psi (t,s) | y (s)^q d\mu(s) \leq [a y (t) ]^q, \quad \quad \mu-\text{p.w.}
		\end{equation}
		oraz
		\begin{equation}\label{eq:zal2}
		\int_T  | \psi (t,s) | y (s)^p d\mu(s) \leq [b y (t) ]^p, \quad \quad \mu-\text{p.w.},
		\end{equation}
		wtedy $\mathit{\Psi}: L^p (T,\mu) \rightarrow L^p(T,\mu)$.
	\end{tw}
	\textcolor{red}{[dodaæ coœ o mierze? $\mu$ i $\mu \times \mu$ a $\lambda$ i $\lambda \times \lambda$]}
	\\\textit{Dowód.} Niech $x \in L^p$ i niech $y$ spe³nia za³o¿enia twierdzenia. Mamy
	\begin{gather}
	\begin{aligned}\label{eq:W1}
	\big|\mathit{\Psi} x (t) \big|
	&= \left| \int_T \psi (t,s) x(s) d\mu(s) \right|
	\leq \int_T | \psi (t,s) | | x(s) | d\mu(s)
	\\& = \int_T  [| \psi (t,s) |^{1/q} y (s) ] \cdot [ | \psi (t,s) |^{1/p} | x(s) | y (s)^{-1}  ] d\mu(s)
	\\&= \int_T  \Big[| \psi (t,s) | y^q (s) \Big]^{1/q} \cdot \Big[ | \psi (t,s) |( | x|/y)^p (s)^{-1}  \Big]^{1/p} d\mu(s)
	\\& \overset{\shortstack{$\scriptstyle \text{nierówn.}$\\ $\scriptstyle \text{H\"{o}ldera}$}}{\underset{\eqref{eq:zal1}}{\leq}} a y (s) \cdot \left[ \int_T | \psi (t,s) | (|x|/y)^p (s) d\mu(s) \right]^{1/p}.
	\end{aligned}
	\end{gather}
	...
	= \left[ \int_T \Big| \int_T \psi (t,s) x(s) d\mu(s) \Big|^p d\mu(t) \right]^{1/p} 
	\\&\overset{\eqref{eq:W1}}{\leq} a \left[ \int_T  y^p(t) \int_T | \psi(t,s) | (|x|/y)^p (s)  d\mu(s) d\mu(t) \right]^{1/p}
	\\&\overset{\text{tw.F.}}{=} a \left[ \int_T (|x|/y)^p (s) \int_T y^p(t) | \psi(t,s) | d\mu(t) d\mu(s) \right]^{1/p}
	\\&\overset{\eqref{eq:zal2}}{\leq} ab \left[ \int_T (|x|/y)^p (s) y^p(s) d\mu(s) \right]^{1/p}
	\\&= ab \left[ \int_T |x(s)|^p d\mu(s) \right]^{1/p}
	= ab \|x\|_p < \infty.
	\end{align*}
	Pokazaliœmy, ¿e $\mathit{\Psi} x \in L^p$, co koñczy dowód. \hfill $\square$
	%
	\vspace{0.35cm}\\Gdy $p=2$ oraz $\mu=\lambda$, wtedy $q=p=2$, \eqref{eq:zal1} $\Leftrightarrow$ \eqref{eq:zal2} i mamy
		\begin{equation*}
		\mathit{\Psi}(x)(t)= \int \psi(t,s) x(s) ds, \quad x \in L^2, \ t \in T.
		\end{equation*}
	Tak okreœlony operator $\mathit{\Psi}$ nazywamy \textbf{operatorem ca³kowym} \textcolor{red}{(dla dowolnego $p$)}, zaœ funkcjê $\psi$ nazywamy \textbf{j¹drem ca³kowym} operatora $\mathit{\Psi}$.
	%
	\vspace{0.35cm}\\\textcolor{red}{[za szybkie przejœcie?]}
	%
