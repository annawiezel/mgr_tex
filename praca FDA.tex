...
\begin{document}
	...
	%
		...
		\\&= \sum_{j=1}^{\infty} \lambda_j^2 \sum_{n=1}^{\infty} \langle e_n , \upsilon_j \rangle \overset{\shortstack{$\scriptstyle \text{to¿samoœæ}$\\ $\scriptstyle \text{Parsevala}$}}{=} \sum_{j=1}^{\infty} \lambda_j^2 \| e_j \| = \sum_{j=1}^{\infty} \lambda_j^2 .
		\end{align*}
		\hfill $\square$
	...
	%
	Przestrzeñ $L^2=L^2(T,\mathcal{B}, \lambda)$ nad pewnym przedzia³em $T \subset \mathbb{R}$ jest zbiorem klas mierzalnych funkcji rzeczywistych ca³kowalnych z kwadratem okreœlonych na $T$, tj.
	...
	\begin{equation*}
	\langle x, y \rangle:= \int_{T} x(t) y(t) dt, \quad x,y \in L^2(T).
	\end{equation*}
	...
	\\Mówimy, ¿e zmienna $X$ jest \textbf{ca³kowalna}, jeœli $\mathbb{E} \left\| X \right\| = \mathbb{E} \left[\int X^2(t) dt \right]^{1/2} <\infty $. Jeœli $X$ jest ca³kowalna, to istnieje jedyna funkcja $\mu \in L^2$ taka, ¿e $\mathbb{E} \langle y, X \rangle = \langle y , \mu \rangle$ dla dowolnej funkcji $y \in L^2$ (zauwa¿my, ¿e wartoœæ oczekiwana jest funkcjona³em liniowym, mo¿emy zatem skorzystaæ z twierdzenia Riesza). Zachodzi $\mu (t) = \mathbb{E} [X(t)]$ dla prawie wszystkich $t \in T$ i funkcjê $\mu$ nazywaæ bêdziemy \textbf{funkcj¹ œredniej}. Ponadto, wartoœæ oczekiwana jest przemienna z operatorami ograniczonymi, tj. jeœli X jest ca³kowalna oraz $\mathit{\Psi} \in \mathcal{L}$, to mamy $\mathbb{E}\mathit{\Psi} (X) = \mathit{\Psi} (\mathbb{E} X)$.
	%
	\begin{df}\cite{B}
		\\\textbf{Operator kowariancji} ca³kowalnej zmiennej funkcjonalnej $X$ o funkcji œredniej $\mu_X$ przyjmuj¹cej wartoœci w przestrzeni funkcyjnej $L^2$ spe³niaj¹cej $\mathbb{E}\left\| X \right\|^2 < \infty $ definiujemy nastêpuj¹co
		\begin{equation*}
	...
	%
	%
	\section{Estymacja wartoœci w³asnych i funkcji w³asnych}\label{r:FPC}
	%
	...
	\end{equation}
	gdzie krzywe $Y_n$, $X_n$ oraz nieobserwowalny b³¹d $\varepsilon_n$ nale¿¹ do przestrzeni Hilberta $L^2(T)$. Operator $\mathit{\Psi}: L^2 \rightarrow L^2$ jest ograniczonym operatorem liniowym, który jest operatorem ca³kowym. J¹dro ca³kowe $\psi(t,s)$ operatora $\mathit{\Psi}$ jest funkcj¹ ca³kowaln¹ z kwadratem na $T \times T$. Równoœæ \eqref{eq:FLM} rozumiemy zatem nastêpuj¹co
	\begin{equation*}
	Y_n(t) = \int \psi (t,s) X_n(s) ds + \varepsilon_n (t), \quad n=1,2,...,N.
	\end{equation*}
	Jak i w przypadku standardowego modelu linowego, funkcjonalny model liniowy wymusza pewne za³o¿enia. Podobnie jak poprzednio, wymagamy, aby zmienna losowa $\varepsilon_n$ opisuj¹ca b³¹d modelu spe³nia³a $\mathbb{E}[ \varepsilon_n ] =0$ oraz aby nie by³a skorelowana ze zmiennymi $X_n$.
	...
	%
	\section{Formalne podstawy \textcolor{red}{[nazwa?]}}
	%
	...
	\\\textcolor{red}{[do opracowania: opis danych!]}
	\begin{figure}[h!]
		\centering
	...
	\end{figure}
	%
	\\\textcolor{red}{[do opracowania: przyk³ad danych]}
	\\Korzystaj¹c z dostêpnego pakietu \textit{fda} (\cite{R})
	\\\textcolor{red}{[do opracowania: punkt po punkcie wed³ug opisu procedury testowej w rozdziale 2]}
