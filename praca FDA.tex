...
\end{df}
%
Operatory kowariancji s¹ operatorami ca³kowymi, czyli
...
%
%
%%% tu: estymatory operatorów kowariancji? %%%
%
%\vspace{2cm}
...
%$$ Y(t) = \int \psi (s,t) X(s) ds + \varepsilon (t), $$
%\item[(ii)] model z odpowiedzi¹ skalarn¹ (ang. \textit{the scalar response model})
%$$ Y = \int \psi (s) X(s) ds + \varepsilon,$$
%\item[(iii)] model z odpowiedzi¹ funkcyjn¹ (ang. \textit{the functional response model})
%$$ Y(t) = \psi (t) x + \varepsilon (t). $$
%\end{enumerate}
Rozwa¿aæ bêdziemy pe³en model funkcjonalny postaci
\begin{equation}\label{eq:FLM}
Y_n = \mathit{\Psi} X_n + \varepsilon_n,
\end{equation}
gdzie krzywe $Y_n$, $X_n$ oraz nieobserwowalny b³¹d $\varepsilon_n$ nale¿¹ do przestrzeni Hilberta $L^2(T)$. Operator $\mathit{\Psi}: \ L^2 \rightarrow L^2$ jest ograniczonym operatorem liniowym, który w szczególnoœci jest równie¿ operatorem ca³kowym, którego j¹dro ca³kowe $\psi(t,s)$ jest funkcj¹ ca³kowaln¹ z kwadratem na $T \times T$. Równoœæ \eqref{eq:FLM} rozumiemy zatem jako
\begin{equation*}
Y_n(t) = \int \psi (s,t) X_n(s) ds + \varepsilon_n (t).
\end{equation*}
%
%
...
%
Jednym z podstawowych testów na efektywnoœæ modelu jest test istotnoœci zmiennych objaœniaj¹cych. Zarówno jak i w przypadku klasycznego modelu liniowego w przypadku funkcjonalnego modelu liniowego badamy zerowanie siê operatora $\mathit{\Psi}$, tj.
\begin{equation*}
...
\begin{lem}\cite{K08}, \cite{B}
	...
	\begin{equation*}
	\limsup \limits_{N \rightarrow \infty} N \mathbb{E} \left\| \nu_k - \hat{\nu}_k \right\|^2 < \infty, \quad \limsup \limits_{N \rightarrow \infty} N \mathbb{E} \left\| u_j - \hat{u}_j \right\|^2 < \infty,
	\end{equation*}
	\begin{equation*}
	\limsup \limits_{N \rightarrow \infty} N \mathbb{E} \left[ \left| \gamma_k - \hat{\gamma}_k \right|^2 \right]  < \infty, \quad \limsup \limits_{N \rightarrow \infty} N \mathbb{E} \left[ \left| \lambda_j - \hat{\lambda}_j \right|^2 \right] < \infty,
	\end{equation*}
