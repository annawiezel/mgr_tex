...
\begin{document}
	...
	W pracy przedstawiona zostanie teoria estymacji elementów losowych przyjmuj¹cych wartoœci w nieskoñczenie wymiarowej oœrodkowej (rzeczywistej) przestrzeni Hilberta. W œrodowisku statystyków przyjê³o siê aby w przypadku, gdy $X(\omega)$ s¹ funkcjami (lub ogólnie krzywymi), takie zmienne nazywaæ \textbf{zmiennymi funkcjonalnymi} (ang. \textit{functional variable}), zaœ obserwacje $\chi$ zmiennej $X$ nazywaæ \textbf{dan¹ funkcjonaln¹} (ang. \textit{functional data}). Statystyki takich obiektów s¹ bardziej skomplikowane ni¿ dla zmiennych losowych przyjmuj¹cych wartoœci w $\mathbb{R}$ lub $\mathbb{R}^n$ ($n \in \mathbb{N}$), dlatego, aby zbudowaæ pojêcia funkcji œredniej oraz operatora kowariancji dla zmiennych tego typu, wprowadzimy najpierw niezbêdne pojêcia z dziedziny operatorów liniowych.
	%
	%
	...
	\end{align*}
	Korzystaj¹c z definicji operatorów $C$ oraz $\Delta$ \eqref{eq:operatory}, za³o¿enia, ¿e $\mathit{\Psi}$ jest operatorem ograniczonym (wiêc komutuje z wartoœci¹ oczekiwan¹) oraz z za³o¿enia o niezale¿noœci miêdzy $X$ a $\varepsilon$ zachodzi (\textcolor{red}{Lemat niezale¿noœæ})
	...
	Statystyka testowa powinna zatem sumowaæ kwadraty iloczynów skalarnych (\ref{eq:delta}). Celem tego rozdzia³u jest udowodnienie Twierdzenia \ref{T1} stanowi¹cego, ¿e (\textcolor{red}{przy za³o¿onym sposobie estymacji wartoœci oraz wektorów w³asnych operatora kowariancji}) statystyka
	...
	%
	Podamy teraz \textcolor{red}{(bez dowodu)} podstawow¹ w³asnoœæ \textcolor{red}{przyjêtego sposobu estymacji wartoœci oraz wektorów w³asnych operatorów kowariancji} - œredniokwadratowy b³¹d estymacji maleje do zera szybciej ni¿ liniowo:
	...
	%
	Bêdziemy korzystaæ z wielowymiarowej wersji Centralnego Twierdzenia Granicznego: \textcolor{red}{Bilignsley}
	
	\begin{tw}\label{CTG_multi} \textcolor{red}{CITE}
	\\Niech $X_n = (X_{n,1}, X_{n,2}, ... ,X_{n,k})$ bêdzie ci¹giem niezale¿nych $k$-wymiarowych wektorów losowych o tym samym rozk³adzie. Za³ózmy, ¿e $\mathbb{E}X_{n,j}^2 < \infty$ dla ka¿dego $j=1,...,k$ oraz oznaczmy wektor œrednich jako $\mu = (\mu_1, ..., \mu_k) := (\mathbb{E}X_{n,1}, ..., \mathbb{E}X_{n,k}),$ oraz macierz kowariancji jako $\Sigma = [\sigma_{ij}]_{i,j=1}^k,\ \sigma_{ij} = Cov(X_{n,i}, X_{n,j}) = \mathbb{E}(X_{n,i} - \mu_i )(X_{n,j} - \mu_j)$. Wówczas ci¹g wektorów losowych
	\[ \frac{X_1 + ... + X_n - n\mu}{\sqrt{n}}\]
	zbiega wed³ug rozk³adu do $k$-wymiarowego rozk³adu normalego o wektorze œrednich $\mu$ i macierzy kowariancji $\Sigma$.
	\end{tw}
	%
	...
	\begin{lem}\label{L2} \cite{K08}, \cite{HK}
		\\Jeœli spe³nione s¹ Za³o¿enia \ref{Z1}, \ref{Z2} i H$_0$, to zachodzi zbie¿noœæ wed³ugo rozk³adu $pq$-wymiarowych wektorów losowych
	...
	\[ \leq \|\upsilon_k\|^2\|u_j\|^2 \cdot (\mathbb{E} \|X_n\|^4)^{1/2} (\mathbb{E}\|\varepsilon_n\|^4)^{1/2}. \]
	Zachodzi 
	...
	Poniewa¿ dla $n \neq n'$ elementy losowe $\varepsilon_n$ oraz $\varepsilon_{n'}$ s¹ niezale¿ne oraz $\mathbb{E}\varepsilon_n = 0$, to z \textcolor{red}{Lematu niezale¿noœæ} mamy $\mathbb{E} \langle \varepsilon_n, \varepsilon_{n'} \rangle = 0$, wiêc
	...
	\end{equation}
	Aby udowodniæ równoœæ \eqref{eq:L4.2}, zauwa¿my, ¿e z Lematu \ref{L1} oraz nierównoœci Czebyszewa dla dowolnego $C>0$ mamy
	...
	\[ P(\| \widehat{\Delta}(\upsilon_k) \| > C) \leq \frac{1}{C} \mathbb{E} \| \widehat{\Delta}(\upsilon_k) \| \leq \frac{1}{C} \mathbb{E} \| \widehat{\Delta}\| \|(\upsilon_k) \| \leq \frac{1}{C} \mathbb{E} \| \widehat{\Delta} \|_{\mathcal{S}}\]
	\[= \frac{1}{C \sqrt{N}} \mathbb{E} \left\| X \right\| \mathbb{E} \left\| \varepsilon_1 \right\| \overset{N \rightarrow \infty}{\longrightarrow} 0, \]
	czyli $\| \widehat{\Delta}(\upsilon_k) \| \stackrel{P}{\longrightarrow} 0$.
	St¹d równoœæ \eqref{eq:L4.2} wynika z Lematu \ref{LP}.
	\\Aby wykorzystaæ takie samo uzasadnienie dla \eqref{eq:L4.3} (skorzystaæ z Lematu \ref{L1}), zauwa¿my, ¿e
	\begin{equation*}
	\sqrt{N} \big\langle \widehat{\Delta} (\hat{\upsilon}_k - \upsilon_k), \hat{u}_j \big\rangle = \sqrt{N} \big\langle  \hat{\upsilon}_k - \upsilon_k, \widetilde{\Delta} (\hat{u}_j) \big\rangle, 
	\end{equation*}
